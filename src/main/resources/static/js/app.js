...
function renderNavAvatar() {
    ...
    const me = State.me;
    if (me && me.avatar) {
        el.innerHTML = '<img src="' + me.avatar + '">';
    } else {
...
function createSessionItem(conv) {
    ...
    if (conv.avatar) {
        avatarDiv.innerHTML = '<img src="' + conv.avatar + '">';
    } else {
...
function renderContacts(container) {
    ...
    State.friends.forEach(f => {
        ...
        if (f.avatar) {
            avatar.innerHTML = '<img src="' + f.avatar + '">';
        } else {
...
function createMessageRow(msg) {
    ...
    if (msg.senderAvatar) {
        avatar.innerHTML = '<img src="' + msg.senderAvatar + '">';
    } else {
    ...
    if (State.currentEmojiTab === 'emoji') {
        ...
        if (State.myEmojis.length === 0) {
            ...
            State.myEmojis.forEach(emoji => {
                ...
                item.innerHTML = '<img src="' + emoji.url + '" title="' + escapeHtml(emoji.name || '') + '">';
    ...
    if (State.activeChat.type === 'GROUP') {
        ...
        if (friend) {
            body.innerHTML = '<div style="text-align:center;padding:16px 0;">' +
                ...
                (friend.avatar ? '<img src="' + friend.avatar + '" style="width:100%;height:100%;object-fit:cover;">' : escapeHtml((friend.nickname || friend.username || '?').charAt(0).toUpperCase())) +
                '</div>' +
